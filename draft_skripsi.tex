...
\thispagestyle{myplain}
	...
	Untuk mencari nilai energi yang dimiliki oleh Graphene, maka dapat digunakan persamaan eigen sesuai dengan Persamaan \ref{schrodinger}. Persamaan eigen menghasilkan vektor tak-nol yang dipetakan oleh transformaasi linier dari \textit{vector space} ke vektor hasil dari perkalian skalar dengan vektor basisnya atau sering disebut sebagai vektor eigen, dan suatu nilai skalar yang terkait dengan transformasi linier yang diberikan dari \textit{vector space} nya atau sering disebut sebagai nilai eigen \cite{MathIsFun}. Persamaan eigen secara matematis ditulis dengan,
	\begin{equation}
	\hat{A} \vec{x} = \lambda \vec{x}
	\end{equation}
	\textit{Block Matrix} merupakan salah satu cara untuk mempercepat kalkulasi persamaan eigen. Dengan menggunakan Block Matrix maka operasi dapat diparalelisasi \cite{Davis2004}. Block Matrix adalah operasi matriks dimana suatu matriks berdimensi besar akan dibagi menjadi beberapa sub matriks kecil \cite{Ford2003}.
	...
	\end{bmatrix}
	\end{equation}
	%Dengan metode Block Matrix akan digunakan untuk mengkalkulasi persamaan eigen. Persamaan eigen akan menghasilkan dua buah output, yaitu nilai eigen dan vektor eigen. Vektor eigen adalah vektor karakteristik tidak nol yang menggambarkan sistem linier dari sebuah persamaan matriks. Nilai eigen adalah nilai skalar untuk mentransformasi vektor eigen atau sering disebut sebagai karakteristik skalar \cite{MathIsFun}. 
...
\begin{center}
	\includegraphics[width=12cm]{gambar/time-gcloud-onegraph.png}
	\captionof{figure}{Grafik Waktu Komputasi terhadap Banyak Atom untuk Server Google Cloud}
	\label{time_gcloud}
\end{center}
\begin{center}
	\includegraphics[width=12cm]{gambar/time-kst-onegraph.png}
...
\begin{equation}
\begin{cases}
y = x^{2} + 0.0566065 x - 6.13494 \ (Python)\\
y = x^{2} + 5.36721 x - 145.615 \ (Multiprocessing)\\
y = x^{2} - 0.135739 x - 0.215598 \ (PySpark) \\
y = x^{2} + 0.067744 x - 6.38048 \ (Threading)
\end{cases}
\end{equation}
dan untuk Server KST,
\begin{equation}
\begin{cases}
y = x^{2} + 0.616388 x - 20.1333 \ (Python)\\
y = x^{2} + 5.56834 x - 133.594 \ (Multiprocessing)\\
y = x^{2} - 0.235929 x + 2.29776 \ (PySpark) \\
y = x^{2} + 0.680441 x - 22.4496 \ (Threading)
\end{cases}
\end{equation}
...
Terdapat puncak-puncak kecil absorpsi setelah puncak terbesarnya. Seiring bertambahnya atom karbon didalam graphene, puncak-puncak kecil tersebut semakin menghilang. Puncak-puncak kecil dari absorpsi ini disebabkan oleh kesalahan komputasi yang diakibatkan ukuran graphene yang dikalkulasi sangat kecil.
